...
namespace Unity.ContentManager
{
    ...
    class ContentDrawer
    {
        ...
        const string k_CategoryLabelID = "CategoryLabel";
        ...
        const string k_BackButtonID = "BackButton";
        const string k_BackButtonLabelID = "BackButtonLabel";

        const string k_ContentDetailID = "ContentDetail";
        const string k_ContentOverviewID = "ContentOverview";

        const string k_UpdatedString = "Last Updated: ";

        const float k_LoadingRotationSpeed = 360.0f;

        const int k_PixelsToDisplayContentDetail = 870;
        const int k_MaxContentDetailWidthSizeInExtendedMode = 320;
        bool m_WindowInCompactMode = false;
        ...
        VisualElement m_ContentDetailView;
        VisualElement m_ContentOverviewView;
        
        VisualElement m_BackButton;
        Label m_BackButtonLabel;
        ...
        void SetupUI()
        {
            ...
            m_RootVisualElement.RegisterCallback<GeometryChangedEvent>(_ => OnUpdateWindowSize());
            ...
            m_ContentDetailView = m_RootVisualElement.Q<VisualElement>(k_ContentDetailID);
            m_ContentOverviewView = m_RootVisualElement.Q<VisualElement>(k_ContentOverviewID);
            ...
            m_BackButtonLabel = m_RootVisualElement.Q<Label>(k_BackButtonLabelID);
            // Set up consistent actions for any of the content installation/configuration buttons
            m_BackButton = m_RootVisualElement.Q(k_BackButtonID);
            if (m_BackButton != null)
            {
                if (m_BackButton is Button asButton)
                    asButton.clicked += BackButtonClicked;
            }
        ...
        void CategoryButtonClicked(PointerDownEvent evt)
        {
            ...
            if (m_WindowInCompactMode)
            {
                m_ContentDetailView.style.display = DisplayStyle.None;
                m_ContentOverviewView.style.display = DisplayStyle.Flex;
                m_SelectedPack = null;
                UpdateContentList();
            }
        }
        ...
        void ContentPackButtonClicked(PointerDownEvent evt)
        {
            ...
            DisplayDetailViewIfNecessary();
            UpdateContentDetail();
        }

        void DisplayDetailViewIfNecessary()
        {
            if (m_RootVisualElement.resolvedStyle.width < k_PixelsToDisplayContentDetail)
            {
                m_ContentDetailView.style.display = DisplayStyle.Flex;
                m_ContentOverviewView.style.display = DisplayStyle.None;
                m_BackButton.style.display = DisplayStyle.Flex;
            }
        }
        ...
        void UpdateContentDetail()
        {
            ...
            if (m_SelectedPack != null)
            {
                if (m_WindowInCompactMode)
                {
                    m_BackButtonLabel.text = m_LastCategoryElement.Q<Label>(k_CategoryLabelID).text;
                }
        ...
        void ProductEntryClicked(PointerDownEvent evt)
        {
            if (GUI.skin == null)
                return;
        ...
        void BackButtonClicked(PointerDownEvent evt)
        {
            BackButtonClicked();
        }
        ...
        {
            m_WindowInCompactMode = m_RootVisualElement.resolvedStyle.width < k_PixelsToDisplayContentDetail;
            if (m_WindowInCompactMode)
            {
                m_ContentDetailView.style.display = DisplayStyle.None;
                m_ContentOverviewView.style.display = DisplayStyle.Flex;
                m_BackButton.style.display = DisplayStyle.Flex;
                m_ContentDetailView.style.width = StyleKeyword.Auto;
            }
            else
            {
                m_ContentDetailView.style.display = DisplayStyle.Flex;
                m_ContentDetailView.style.width = k_MaxContentDetailWidthSizeInExtendedMode;
                m_ContentOverviewView.style.display = DisplayStyle.Flex;
                m_BackButton.style.display = DisplayStyle.None;
            }
