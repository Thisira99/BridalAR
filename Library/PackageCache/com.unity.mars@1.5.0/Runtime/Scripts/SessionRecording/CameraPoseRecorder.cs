    ...
    public class CameraPoseRecorder : DataRecorder, IUsesCameraPose, IUsesSlowTasks
    {
        ...
        public override DataRecording TryCreateDataRecording(TimelineAsset timeline, List<UnityObject> newAssets)
        {
            ...
            for (var i = 0; i < eventsCount; i++)
            {
                ...
                // Unroll rotation to ensure shortest path
                if (i > 0)
                {
                    var previousIndex = i - 1;
                    var previous = new Quaternion(xRotCurve[previousIndex].value, yRotCurve[previousIndex].value,
                        zRotCurve[previousIndex].value, wRotCurve[previousIndex].value);
                    rotation = Quaternion.Dot(previous, rotation) < 0.0f ?
