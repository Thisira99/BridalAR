...
namespace Unity.MARS
{
    ...
    static class Entitlements
    {
        ...
        internal static bool DisableUnityProBypass {  get; set; }
        ...
        public static bool HasRequest => Status.HasFlag(EntitlementStatus.HasReceivedResponse);

        static bool HasFreeEntitlements => Status.HasFlag(EntitlementStatus.HasFreeEntitlements);
        ...
        public static bool IsAccountSignedIn => Status.HasFlag(EntitlementStatus.HasAccessToken);
        ...
        static void ReceiveEntitlements(string licenceInfo, bool marsEntitlementOverride = false)
        {
            var wasEntitled = IsEntitled;
            Status &= ~EntitlementStatus.HasAnyEntitlements;
            Status |= !marsEntitlementOverride ? CheckForSubscriptionPlan(licenceInfo) : EntitlementStatus.HasBaseEntitlements;
        ...
        {
            // Internal users are entitled by default
            ...
            // Pro users are entitled by default
            if (!DisableUnityProBypass && UnityEditorInternal.InternalEditorUtility.HasPro())
            {
                ReceiveEntitlements(null, true);
                return;
            }
