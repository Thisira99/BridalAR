...
namespace UnityEditor.MARS.CodeGen
{
    ...
    public static class TraitCodeGenerator
    {
        ...
        internal static void TryRunGeneration()
        {
            ...
            CheckSynthesizedTraitTypes(conditionTypes, relationTypes);
        ...
        static void CheckSynthesizedTraitTypes(CodeGenerationTypeData[] conditionTypes, CodeGenerationTypeData[] relationTypes)
        {
            k_TempTypes.Clear();
            typeof(SynthesizedTrait).GetAssignableTypes(k_TempTypes);
            foreach (var synthTraitType in k_TempTypes)
            {
                var genericBaseType = FindGenericSynthesizedTraitBaseType(synthTraitType);
                var args = genericBaseType.GetGenericArguments();
                if (args.Length == 0)
                    continue;

                var userDataType = args[0];
                var hasConditionType = conditionTypes.Any(conditionType => conditionType.Type == userDataType);
                var hasRelationType = relationTypes.Any(relationType => relationType.Type == userDataType);
                if (!hasConditionType && !hasRelationType)
                {
                    XRToolsLoggingUtils.LogError($"SynthesizedTrait exists for type {userDataType.FullName} but no " +
                                                 "Condition or Relation is defined for this trait type. MARS will only generate " +
                                                 "code for this trait type if there is a Condition or Relation that uses it.");
                }
            }
        }

        static Type FindGenericSynthesizedTraitBaseType(Type type)
        {
            var synthTraitType = typeof(SynthesizedTrait);
            if (type == synthTraitType || !synthTraitType.IsAssignableFrom(type))
                return null;

            var baseType = type.BaseType;
            while (baseType != synthTraitType && baseType != null)
            {
                var nextBaseType = baseType.BaseType;
                if (nextBaseType == synthTraitType) // base type of SynthesizedTrait<T> is SynthesizedTrait
                    return baseType;
