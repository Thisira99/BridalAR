...
namespace Unity.MARS.XRSubsystem
{
    class MarsXRSubsystemModule : IModuleDependency<MARSSceneModule>, IModuleBehaviorCallbacks
    {
        ...
        /// <summary>
        /// True if any subsystems in use when the scene was loaded.
        /// </summary>
        static public bool SubsystemsInUse { get; private set; }
        ...
        static void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            SubsystemsInUse = anyMarsSubsystemsRunning;

            if (!anyMarsSubsystemsRunning)
            {
                k_XRMeshSubsystems.Clear();
                SubsystemManager.GetInstances(k_XRMeshSubsystems);
                anyMarsSubsystemsRunning = k_XRMeshSubsystems.Any(x => x.running && x.SubsystemDescriptor.id == MARSXRSubsystemLoader.k_MeshSubsystemId);
            }
        ...
        public void ConnectDependency(MARSSceneModule dependency)
        {
            ...
            k_XRMeshSubsystems.Clear();
            SubsystemManager.GetInstances(k_XRMeshSubsystems);
            var meshSubsystem = k_XRMeshSubsystems.FirstOrDefault(x => x.running && x.SubsystemDescriptor.id == MARSXRSubsystemLoader.k_MeshSubsystemId);
